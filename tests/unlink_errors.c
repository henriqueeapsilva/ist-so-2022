...
int main() {
...

    {
        ...
        It is not an error though. The file should be removed from its directory
        and the links count should be decremented, but the inode is intact so
        that it can be used by IO operations until no more instances of open
        files remain.
        */
        uint8_t buffer[sizeof(file_contents)];
        int f1;
