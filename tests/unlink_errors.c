...
int main() {
  ...
  {
    /*
    Scenario 1: Unlinking open files should be postponed to when they are
    closed.

    It is not an error though. The file should be removed from its directory and
    the links count should be decremented, but the inode is intact so that it
    can be used by IO operations until no more instances of open files remain.
    */
    uint8_t buffer[sizeof(file_contents)];
    int f1;
    int f2;
    ...
    assert(tfs_unlink(target_path) !=
           -1); // remove from directory but don't delete inode yet
    assert(tfs_open(target_path, TFS_O_TRUNC | TFS_O_APPEND) ==
           -1); // we should not be able to open it anymore

    w = tfs_write(f1, file_contents,
                  sizeof(file_contents)); // we can still write content though
    assert(w == sizeof(file_contents));

    assert(tfs_open(target_path, TFS_O_CREAT) !=
